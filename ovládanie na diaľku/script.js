$(document).ready(() => {
    ...
    // Menu
    menuButton.click(() => {
        screenshareIntervalInput.val(screenshareInterval);
        mouseUpdateIntervalInput.val(mouseUpdateInterval);
        if (menuState == 0) {
            menu.css("left", "0");
            menuState = 1;
        } else {
    ...
    saveMenu.click(() => {
        screenshareInterval = screenshareIntervalInput.val();
        mouseUpdateInterval = mouseUpdateIntervalInput.val();
        clearInterval(updateScreen);
        if (screenshareInterval != 0) {
            updateScreen = setInterval(() => {
                screenshare();
    ...
    keyboardPreviousValue = keyboard.val();
    ...
    keyboard.on("input", () => {
        keyboardCurrentValue = keyboard.val()
        if (keyboardCurrentValue === "" && keyboardPreviousValue === "") {
            post("Backspace", "hotkeyPress");
        }
        keyboardPreviousValue = keyboardCurrentValue
    });
    ...
    //     let keyboardValue = keyboard.val();
        
    //     if (keyboardValue === "") {
    //         post("Backspace", "hotkeyPress");
    //     } else {
    //         post(keyboard.val().slice(1), "textWrite");
    //     }
